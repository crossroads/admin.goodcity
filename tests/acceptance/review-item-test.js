...
var App, role, offer, item, packageType, package1, package2, orders_pkg1, packages_location;
...
module('Display review Item', {
  beforeEach: function() {
    ...
    package1 = FactoryGuy.make("package", { item: item, packageType: packageType, notes: "" });
    package2 = FactoryGuy.make("package", { item: item, packageType: packageType, notes: "new edited" });
...
test("Assign description same as the assigned Package Name for the first time", function(assert) {
  ...
  andThen(function() {
    assert.equal(currentURL(), "/offers/"+offer.id+"/item/"+ item.id +"/search_label");
